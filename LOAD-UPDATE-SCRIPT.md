# How to Run updateServicesWithEnglish() in Browser

## Quick Method - Paste in Console:

```javascript
// Step 1: Load the script
const script = document.createElement('script');
script.src = 'js/update-services-english-browser.js';
script.onload = () => console.log('✅ Script loaded! Now run: updateServicesWithEnglish()');
script.onerror = () => console.error('❌ Failed to load script. Make sure you\'re on the website.');
document.head.appendChild(script);

// Step 2: After script loads, run:
// updateServicesWithEnglish()
```

## Alternative - Direct Paste (All-in-One):

Copy and paste this complete code into your browser console:

```javascript
(async function() {
    if (!window.firebaseDB || !window.getServicesCollection) {
        console.error('❌ Firebase not initialized. Please load the website first.');
        return;
    }
